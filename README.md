## Hi there 👋

# Hi there, I'm robyn 👋

### 🧑‍💻 About Me
**"끊임없이 성장하는 개발자 [본인 한글 이름/닉네임]입니다."**

- 🎓 **Major**: Computer science
- 🔭 **Interested in**: Compiler
- 🌱 **Currently Learning**: [현재 배우고 있는 것] (예: Verilog, Python, Network)
- 💬 **Contact**: [이메일 주소]

---

### 🛠️ Tech Stacks
<div align="left">
	<img src="https://img.shields.io/badge/C++-00599C?style=flat&logo=c%2B%2B&logoColor=white" />
	<img src="https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white" />
	<img src="https://img.shields.io/badge/Verilog-B02927?style=flat&logo=v&logoColor=white" />
</div>

---

### 🚀 Current Projects & Roles
**현재 진행 중이거나 완료한 주요 프로젝트입니다.**

| 프로젝트명 (Link) | 핵심 설명 및 목적 | 맡은 역할 (Role) |
| :--- | :--- | :--- |
| **[프로젝트 이름 A](레포지토리_URL)** | **[한 줄 핵심 설명]**<br>ex) 그래프 탐색 알고리즘을 활용한 미로 찾기 게임<br>• **목적**: 알고리즘 시각화 및 게임 로직 구현 학습 | **[본인의 역할]**<br>• 백엔드 로직 구현 (Python)<br>• 미로 생성 알고리즘(DFS) 작성<br>• 결과 리포트 작성 |
| **[프로젝트 이름 B](레포지토리_URL)** | **[한 줄 핵심 설명]**<br>ex) FPGA를 활용한 디지털 시계 구현<br>• **목적**: 하드웨어 기술 언어(HDL) 이해도 증진 | **[본인의 역할]**<br>• Verilog 모듈 설계 (Counter, Divider)<br>• Testbench 작성 및 시뮬레이션 검증 |

---

### 📈 GitHub Stats & Activity
<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=[본인ID]&show_icons=true&theme=radical" height="150" alt="stats graph" />
  
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=[본인ID]&layout=compact&theme=radical" height="150" alt="languages graph" />
</div>

<br>

<div align="center">
  <img src="https://github-readme-snake.svgomg.com/github-readme-snake.svg" alt="snake animation" />
</div>

---

### 🗺️ Dev Roadmap (Future Goal)
앞으로 다음과 같은 기술을 학습하여 전문성을 넓혀갈 계획입니다.

- [ ] **Short-term**: [단기 목표] (예: 리눅스 커널 구조 파악, 정보처리기사 취득)
- [ ] **Long-term**: [장기 목표] (예: 임베디드 시스템 전문가, 풀스택 개발자)<!--
**nodevvice/nodevvice** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
